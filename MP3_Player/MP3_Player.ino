/**
   Date: May 16th, 2023
   Description: uses MP3Shield Arduino driver, and listens for commands from a serial terminal (like the Serial
   ...
   2 will play 'track002.mp3'. A lower case 's' will stop playing the mp3.
...
// Create a constant variable for the arduino pin
const int pausePin = 13;

void setup() {

  // Sets the button pin to input
...
// Variable to determine if it is the first time running the program
bool firstTime = true;
...
void loop() {
  ...
  if (Serial.available() || firstTime) {
    
    // If it is the first time increase the volume to max
    if (firstTime) {
      // Loop to set the dB volume to max
      for (int i = 0; i < 19; i++) {
        parse_menu('+');
      }
